...
// variables to add resposnsiveness when the screen size is large
...
const interval_id = setInterval(() => {
  ...
  if (isPlayerTouchingElement(player, object)) {
    ...
    sessionStorage.setItem("play_sound1",1);
    window.location.href = "screen4.html";
  }

  // for every 200 score a powerup traverses on screen from right to left
  if (score%200== 0) {
...
function saveScoreToLocalStorage(score) {
  ...
  // updating the score of this particular user, only if its greater than all the previous scores.
